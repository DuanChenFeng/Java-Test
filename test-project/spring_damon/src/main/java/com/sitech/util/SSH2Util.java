...
public class SSH2Util {
	...
	* @throws
	...
	public static void scpPut(String ip, String username, String password, 
			...
			throws IOException {
		logger.info("ConnectLinuxCommand  scpPut===" + "ip:" + ip + 
				"  userName:" + username + "  localFile:"
	...
	public static void run_ssh() throws IOException, SQLException {
		ConnInfo connInfo = new ConnInfo();
		//插入连接信息
		...
		String cmd_top = "cd ~/mydata; cat top";
		String out_df = "src\\main\\data\\df.txt";
		String cmd_df = "cd ~/mydata; cat df";
		String out_uname = "src\\main\\data\\uname.txt";
		String cmd_uname = "cd ~/mydata; uname -a";
		String out_dn = "src\\main\\data\\dn.txt";
		String cmd_dn = "cd ~/mydata; cat hadoop";
		...
		if (login == true) {
			...
			String res_dn = SSH2Util.execute(cmd_dn);
			DataCleaning.write_file(out_dn, res_dn);
			
		}else {
			System.out.println("ERROR!!!");
		}
		//对得到的数据进行提取并存入对应的数据库表中
